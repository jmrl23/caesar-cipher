 ...
 */
const loopedArray = <T>(collection: T[], index: number): any => {
  index = Math.floor(index)
  if (index < 0) return collection[collection.length + index]
  if (collection.length > index) return collection[index]
 ...
 */
const caesarCipher = (word: string, shift: number): string => {
  ...
  for (const character of word) {
    ...
    result.push(loopedArray<string>(characters, index + shift))
  }
  return result.join('')
}
